...
namespace ThinkingData.Analytics
{
    ...
    public class TDAnalytics
    {
        ...
        public static bool Init(TDConfig config)
        {
            ...
            return InitWrapper(config.AppId, config.ServerUrl, config.EnableEncrypt, config.Version, config.PublicKey, (int)config.Mode, config.DatabaseLimit, config.DataExpression, config.DatabasePath, zoneOffsetStr, config.EnableAutoCalibrated);
        }
        ...
        public static void Track(TDFirstEventModel firstEvent)
        {
            if (firstEvent != null)
            {
                firstEvent.Check();
                var properties = JsonConvert.SerializeObject(firstEvent.properties, settings);
                TrackFirstEventWrapper(firstEvent.eventName, properties, firstEvent.firstCheckId);
            }
            else
            {
                TDLog.Print(LogLevel.Error, TDErrorMsg.ParamIsNull);
        ...
        public static void Track(TDUpdatableEventModel updatableEvent)
        {
            if (updatableEvent != null)
            {
                updatableEvent.Check();
                var properties = JsonConvert.SerializeObject(updatableEvent.properties, settings);
                TrackUpdatableEventWrapper(updatableEvent.eventName, properties, updatableEvent.eventId);
            }
            else
            {
                TDLog.Print(LogLevel.Error, TDErrorMsg.ParamIsNull);
        ...
        public static void Track(TDOverwritableEventModel overWritableEvent)
        {
            if (overWritableEvent != null)
            {
                ...
                TrackOverWritableEventWrapper(overWritableEvent.eventName, properties, overWritableEvent.eventId);
            }
            else
            {
                TDLog.Print(LogLevel.Error, TDErrorMsg.ParamIsNull);
